...
import { randomInt } from "crypto"; // Или другой способ генерации случайных чисел

it("real ticks with variance", async () => {
    const NUM_SAMPLES = 100; // Количество итераций для каждого количества тиков
    const TICK_SPACING = 100n; // Базовый интервал между тиками
    const MAX_TICKS = 100; // Максимальное количество диапазонов для теста
    ...
    // Для каждого количества диапазонов от 1 до MAX_TICKS
    for (let numRanges = 1; numRanges <= MAX_TICKS; numRanges++) {
        let gasResults: number[] = [];

        // 1000 итераций для этого количества диапазонов
        for (let sample = 0; sample < NUM_SAMPLES; sample++) {
            ...
            // Создание рандомизированных диапазонов ликвидности
            for (let range = 0; range < numRanges; range++) {
                const baseTick = range * Number(TICK_SPACING);
                const randOffset = BigInt(randomInt(0, 20)); // Случайное смещение до 20 тиков
            ...
            // Выполнение свапа через все диапазоны
            ...
            const swap = await pool.send(
                ...
                {value: toNano("1")},
                {
                    ...
                    userAddress: user.address,
                    zeroForOne: false,
                    amountSpecified: 5000000000000n,
        ...
        // Расчет среднего и дисперсии для этого количества диапазонов
